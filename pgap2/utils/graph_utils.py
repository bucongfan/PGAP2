...
def merge_node(G: nx.Graph, pg, tree: Tree, sources: list, target: str):
    ...
    # Propagate has_new: if any source node has new genes, the merged node does too
    if 'has_new' in G.nodes[target]:
        G.nodes[target]['has_new'] = any(
            G.nodes[n].get('has_new', False) for n in nodes if G.has_node(n))
